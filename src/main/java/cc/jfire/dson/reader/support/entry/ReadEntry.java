...
public interface ReadEntry
{
    ...
    static ReadEntry standard(String name, Field field, Type ownerType, ReaderContext readerContext)
    {
        return new StandardReadEntry(name, field, ownerType, readerContext);
    }

    @SneakyThrows
    static ReadEntry compile(Field field, Type ownerType, ReaderContext readerContext)
    {
        TypeReader typeReader = null;
        Type       resolvedType = readerContext.resolveType(ownerType, field.getGenericType());
        int        classId      = resolvedType instanceof Class<?> resolvedClass ? ReflectUtil.getClassId((Class) resolvedClass) : ReflectUtil.getClassId(field.getType());
        ...
        if (typeReader != null)
        {
            switch (classId)
            {
                case ReflectUtil.PRIMITIVE_BYTE, ReflectUtil.CLASS_BYTE -> builder.append(STR.format("(({})instance).{}((Byte)typeReader.fromString(stream));", typeName, methodName));
                case ReflectUtil.PRIMITIVE_INT, ReflectUtil.CLASS_INT -> builder.append(STR.format("(({})instance).{}((Integer)typeReader.fromString(stream));", typeName, methodName));
                case ReflectUtil.PRIMITIVE_SHORT, ReflectUtil.CLASS_SHORT -> builder.append(STR.format("(({})instance).{}((Short)typeReader.fromString(stream));", typeName, methodName));
                case ReflectUtil.PRIMITIVE_LONG, ReflectUtil.CLASS_LONG -> builder.append(STR.format("(({})instance).{}((Long)typeReader.fromString(stream));", typeName, methodName));
                case ReflectUtil.PRIMITIVE_FLOAT, ReflectUtil.CLASS_FLOAT -> builder.append(STR.format("(({})instance).{}((Float)typeReader.fromString(stream));", typeName, methodName));
                case ReflectUtil.PRIMITIVE_DOUBLE,
                     ReflectUtil.CLASS_DOUBLE -> builder.append(STR.format("(({})instance).{}((Double)typeReader.fromString(stream));", typeName, methodName));
                case ReflectUtil.PRIMITIVE_BOOL, ReflectUtil.CLASS_BOOL -> builder.append(STR.format("(({})instance).{}((Boolean)typeReader.fromString(stream));", typeName, methodName));
                case ReflectUtil.PRIMITIVE_CHAR,
                     ReflectUtil.CLASS_CHAR -> builder.append(STR.format("(({})instance).{}((Character)typeReader.fromString(stream));", typeName, methodName));
