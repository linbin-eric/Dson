...
public class ArraySerializeDescriptor implements SerializeDescriptor
{
	...
	{
		void initialize(Type type, Map<Type, SerializeDescriptor> map);
	...
	public void initialize(Serializer serializer, Type type, Map<Type, SerializeDescriptor> map)
	{
		...
		arraySerializer.initialize(type, map);
	}
	...
	abstract class TemplateSerializer implements ArraySerializer
	{
		...
		@Override
		public void initialize(Type type, Map<Type, SerializeDescriptor> map)
		{
		}
	...
	class FinalArraySerializer extends TemplateSerializer
	{
		...
		public void initialize(Type type, Map<Type, SerializeDescriptor> map)
		{
			super.initialize(type, map);
			if (type instanceof Class<?>)
			{
				serializeDescriptor = serializer.describe(((Class<?>) type).getComponentType(), map);
			}
			else if (type instanceof GenericArrayType)
			{
				serializeDescriptor = serializer.describe(((GenericArrayType) type).getGenericComponentType(), map);
			}
			else
			{
				throw new IllegalArgumentException();
