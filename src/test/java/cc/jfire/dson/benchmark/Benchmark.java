package cc.jfire.dson.benchmark;
...
public class Benchmark
{
    ...
    public void bigParse() throws JsonParseException, JsonMappingException, IOException
    {
        ...
        timewatch.end();
        logger.info("fastjson大json反序列化耗时：{}", timewatch.getTotal());
        timewatch.start();
        for (int i = 0; i < count; i++)
        {
            Dson.fromString(BigData.class, value);
        }
        timewatch.end();
        logger.info("codejson大json反序列化耗时：{}", timewatch.getTotal());
        timewatch.start();
        for (int i = 0; i < count; i++)
        {
        ...
        timewatch.end();
        logger.info("jackson2大json反序列化耗时：{}", timewatch.getTotal());
    }
}
